 ...
 */
	...
	#ifndef RNDIS_DEVICE_H_
	#define RNDIS_DEVICE_H_
...
#pragma pack(push, 4)
	...
	{
		uint8_t Packets[Eth_RNDIS_Rx_Queue_Len][EMAC_ETH_MAX_FLEN + sizeof(RNDIS_Packet_Message_t)];
		uint8_t OWN_flag[Eth_RNDIS_Rx_Queue_Len]; //if 1 still don't readed by main
		uint16_t bw;
		uint16_t br;
		uint32_t RX_buf_queued;
	}RNDIS_RX_Queue_t;

	typedef struct RNDIS_TX_Queue_struct
	{
		uint8_t Packets[Eth_RNDIS_Tx_Queue_Len][EMAC_ETH_MAX_FLEN + sizeof(RNDIS_Packet_Message_t)];
		uint8_t OWN_flag[Eth_RNDIS_Tx_Queue_Len];  //if 1 OWN by USB Stack
		uint16_t bw;
		uint16_t br;
		uint32_t tx_busy;
	}RNDIS_TX_Queue_t;
